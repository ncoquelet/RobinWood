...
import { LABEL_1, LABEL_2, UNKNOWN_LABEL_ID } from '../scripts/utils/constants'
import {
  deployLabelContract,
...
describe('contract', function () {
  ...
  describe('Label', function () {
    ...
    it('Should admin accept new label', async () => {
      const { labelC, cert1 } = await loadFixture(withSubmitedCertifierLabel)
    ...
    it('Should verify if label is allowed', async () => {
      const { labelC, cert1, pub } = await loadFixture(
        withAllowedCertifierLabel
      )
  ...
  describe('Transferable', function () {
    it('Shoul revert on transfer', async () => {
      const { labelC, cert1, cert2 } = await loadFixture(
        withAllowedCertifierLabel
      )
